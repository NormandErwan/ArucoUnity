...
namespace ArucoUnity.Objects.Trackers
{
    public class ArucoMarkerTracker : ArucoObjectTracker
    {
        ...
        /// <summary>
        /// Combines the data from MarkerIds, MarkerTvecs, and MarkerRvecs.
        /// It is locked on EstimateTransforms and Updatetransforms methods to avoid race conditions.
        /// </summary>
        public Dictionary<Aruco.Dictionary, List<(int markerID, Vector3 position, Quaternion rotation)>>[] MarkersData { get; protected internal set; }
        ...
        protected override void ArucoObjectsController_DictionaryAdded(Aruco.Dictionary dictionary)
        {
            for (int cameraId = 0; cameraId < arucoCamera.CameraNumber; cameraId++)
            {
                ...
                MarkersData[cameraId].Add(dictionary, new List<(int markerID, Vector3 position, Quaternion rotation)>());
            }
        }
        ...
        protected override void ArucoObjectsController_DictionaryRemoved(Aruco.Dictionary dictionary)
        {
            for (int cameraId = 0; cameraId < arucoCamera.CameraNumber; cameraId++)
            {
                ...
                MarkersData[cameraId].Remove(dictionary);
            }
        }
        ...
        public override void Activate(IArucoObjectsTracker arucoTracker)
        {
            ...
            MarkersData = new Dictionary<Aruco.Dictionary, List<(int markerID, Vector3 position, Quaternion rotation)>>[arucoCamera.CameraNumber];
            ...
            for (int cameraId = 0; cameraId < arucoCamera.CameraNumber; cameraId++)
            {
                ...
                MarkersData[cameraId] = new Dictionary<Aruco.Dictionary, List<(int markerID, Vector3 position, Quaternion rotation)>>();
        ...
        public override void EstimateTransforms(int cameraId, Aruco.Dictionary dictionary)
        {
            ...
            //Update list of markers to update
            lock (MarkersData[cameraId][dictionary])
            {
                MarkersData[cameraId][dictionary].Clear();

                for (uint i = 0; i < DetectedMarkers[cameraId][dictionary]; i++)
                    MarkersData[cameraId][dictionary].Add(
                            (
                                MarkerIds[cameraId][dictionary].At(i),
                                MarkerTvecs[cameraId][dictionary].At(i).ToPosition(),
        ...
        public override void UpdateTransforms(int cameraId, Aruco.Dictionary dictionary)
        {
            ...
            if (MarkersData[cameraId][dictionary] != null)
                lock (MarkersData[cameraId][dictionary])
                {
                    for (uint i = 0; i < MarkersData[cameraId][dictionary].Count; i++)
                    {
                        int index = (int)i;

                        ArucoObject foundArucoObject;
                        int detectedMarkerHashCode = ArucoMarker.GetArucoHashCode(MarkersData[cameraId][dictionary][index].markerID);
                        if (arucoTracker.ArucoObjects[dictionary].TryGetValue(detectedMarkerHashCode, out foundArucoObject))
                        {
                            var localPosition = MarkersData[cameraId][dictionary][index].position * foundArucoObject.MarkerSideLength / estimatePoseMarkerLength;
                            arucoCameraDisplay.PlaceArucoObject(foundArucoObject.transform, cameraId, localPosition,
                              MarkersData[cameraId][dictionary][index].rotation);
                        }
                    }
                }
        }
