  ...
  namespace Examples
  {
    public class Marker : MonoBehaviour
    {
      ...
      public Utility.Mat Create(PREDEFINED_DICTIONARY_NAME dictionaryName, int markerId, int markerSize, int markerBorderBits)
      {
        ...
        Utility.Mat marker = new Utility.Mat();
      ...
      public Texture2D CreateTexture(Utility.Mat marker)
      {
        return new Texture2D(marker.cols, marker.rows, TextureFormat.RGB24, false);
      }

      public void Draw(Utility.Mat marker, GameObject markerPlane, Texture2D markerTexture)
      {
