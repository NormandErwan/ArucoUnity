  ...
  namespace Plugin
  {
    namespace std
    {
      public class VectorPoint2f : HandleCppPtr
      {
        ...
        {
          au_std_vectorPoint2f_delete(cvPtr);
        }

        public Point2f At(uint pos)
        {
          Exception exception = new Exception();
          Point2f element = new Point2f(au_std_vectorPoint2f_at(cvPtr, pos, exception.cvPtr), DeleteResponsibility.False);
          exception.Check();
          return element;
        }
        ...
        {
          System.IntPtr* dataPtr = au_std_vectorPoint2f_data(cvPtr);
          uint size = Size();
        ...
        {
          au_std_vectorPoint2f_push_back(cvPtr, value.cvPtr);
        }

        public uint Size()
        {
          return au_std_vectorPoint2f_size(cvPtr);
        }
      }
    }
  }
